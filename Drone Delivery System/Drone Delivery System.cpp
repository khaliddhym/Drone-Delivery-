...
ostream& operator<<(ostream& out, const Drone& d) {
	return out << "Drone member  data: " << d.id << ", " << d.capacity << ", " << d.current_load << endl;
}
...
int main()
{
	...
	do
	{
		...
		{
			cout << "Error drone current load plus the weight of the package exceeds its capacity. " << endl;
			cout << "current load plus the   weight:  " << total.putdata() + current_load << endl;
